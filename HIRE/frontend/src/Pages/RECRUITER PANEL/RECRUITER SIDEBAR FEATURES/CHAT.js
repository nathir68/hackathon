...
import { HiOutlineSearch, HiOutlinePaperAirplane, HiOutlinePaperClip, HiOutlineEmojiHappy } from 'react-icons/hi';
...
const RecruiterChat = () => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            // Optionally refetch conversations to bump to top
            const res = await getConversations();
            setConversations(res.data);
        } catch (err) {
    ...
    return (
        <DashboardLayout sidebarLinks={recruiterLinks} userName={(JSON.parse(localStorage.getItem("hire_user") || '{"name":"Guest"}').name)} userRole="Recruiter">
            <div className="page-wrapper">
                <div className="page-header"><h1>Chat</h1></div>
                <div className="glass-card" style={{ padding: 0, overflow: 'hidden', display: 'grid', gridTemplateColumns: '300px 1fr', height: 'calc(100vh - 180px)' }}>
                    <div style={{ borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ padding: '16px', borderBottom: '1px solid var(--border-color)' }}><div className="search-bar"><HiOutlineSearch className="search-icon" /><input placeholder="Search contacts..." /></div></div>
                        <div style={{ flex: 1, overflowY: 'auto' }}>
                            {loading ? <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)' }}>Loading...</div> :
                                conversations.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: 'var(--grey-500)' }}>No active conversations</div> :
                                    conversations.map((c) => (
                                        <div key={c._id} onClick={() => setActiveId(c.userId)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', cursor: 'pointer', borderBottom: '1px solid var(--border-color)', background: activeId === c.userId ? 'var(--bg-glass)' : 'transparent' }}>
                    ...
                    {activeConvo ? (
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '12px' }}><div className="avatar">{activeConvo.name[0]}</div><div><div style={{ fontWeight: 600, fontSize: 'var(--fs-sm)' }}>{activeConvo.name}</div><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{activeConvo.online ? 'Online' : 'Offline'}</div></div></div>
                            ...
                            <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border-color)', display: 'flex', gap: '8px', alignItems: 'center' }}>
                                <button className="btn-icon"><HiOutlinePaperClip /></button>
                                <button className="btn-icon"><HiOutlineEmojiHappy /></button>
                                ...
                                <button className="btn btn-primary" onClick={handleSend}><HiOutlinePaperAirplane /></button>
                            </div>
                        </div>
                    ) : (
